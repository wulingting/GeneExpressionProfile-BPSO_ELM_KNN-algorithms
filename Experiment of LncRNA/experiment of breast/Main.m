...
data=dealdata;
Dim=201;%样本数量
out=randperm(Dim)';%产生1-201无重复的随机数排列
trainnum=101;%训练集数量
validationnum=50;%验证集数量
testnum=50;%测试集数量
...
%分为样本和验证
for i=1:trainnum
traindata(:,i)=dealdata(:,train(i));
end
for i=1:validationnum
validationdata(:,i)=dealdata(:,validation(i));
end
for i=1:testnum
testdata(:,i)=dealdata(:,test(i));
end
traindata=traindata';%这个是什么意思
validationdata=validationdata';
testdata=testdata';
for i=1:trainnum
   if traindata(i,1)>=91 %91是什么指标
       traindata(i,1)=1;
   else 
       traindata(i,1)=0;
   end 
...
while iter<MaxIt
   ...
   for z=1:yznum%1-20
    ...
    for i=1:Dim%1-200
       if(x(xSizenum,i)==1)
            dimnum=dimnum+1;
           validation(z,dimnum)=validationdata(z,i);  
       end
    end
   end
  %设置验证集初始化
...
fx(xSizenum)=testsum;
  ...
  for i=1:xSize
    ...
    for i=1:Dim%1-200
       if(gbest(i)==1)
            dimnum=dimnum+1;
           lasttrain(z,dimnum)=traindata(z,i);  
       end
    end
   end
   ...
   for z=1:yznum%1-20
    ...
    for i=1:Dim%1-200
       if(gbest(i)==1)
            dimnum=dimnum+1;
           lastvalidation(z,dimnum)=validationdata(z,i);  
       end
    end
   end
    ...
    for i=1:Dim%1-200
       if(gbest(i)==1)
            dimnum=dimnum+1;
           lasttest(z,dimnum)=testdata(z,i);  
       end
    end
   end
...
t2=clock;
testtime111 = etime(t2,t1); 
save time  testtime111;
%     for i=1:ybnum
%       lasttrain(i,1)=traindata(i,1);
%     end
%     %最后训练初始化
