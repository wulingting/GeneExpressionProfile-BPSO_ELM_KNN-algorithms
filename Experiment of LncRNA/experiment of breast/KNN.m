...
LoadingTime=0;TrainingTime=0;TestingTime=0;TrainingAccuracy=0;TestingAccuracy=0;
testsum=0;
 train_data=train(:,2:Keygenenum);
 train_label=train(:,1);
 validation_data=validation(:,2:Keygenenum);
 validation_label=validation(:,1);

for testx=1:10
 mdl = ClassificationKNN.fit(train_data,train_label,'NumNeighbors',1);  
 predict_label   =       predict(mdl, validation_data);  
 accuracy         =       length(find(predict_label == validation_label))/length(validation_label)*100;  
   testsum=testsum+accuracy;
end
fx(xSizenum)=testsum;
 end
    %求出elm
