...
testdata=testdata';
for i=1:trainnum
   if traindata(i,1)>=134 %转置之后的矩阵第一列为样本的标记数，需要进行类别标记；1和0
       traindata(i,1)=1;
   else 
       traindata(i,1)=0;
   end 
...
%% 初始化程序:
Keygenenum=10;%关键的lncRNA数量
Dim=200;%粒子的维数，200个样本数量
xSize=10;%种群数  
MaxIt=30;%最大迭代次数
c1=2;
c2=2;%定义加速因子
...
fx(xSizenum)=testsum;
  ...
  for i=1:xSize
      ...
      su=su+x(i,sunum);
        ...
        if su>Keygenenum
            fx(i)=0;%当被包内物品的体积超过限制时,将其适应度设置为1
        end
  end
